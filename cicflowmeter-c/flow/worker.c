...
static void FlowWorkerFlowTimeout(ThreadVars *tv, Packet *p,
                                  FlowWorkerThreadData *fw,
                                  void *detect_thread);
Packet *FlowForceReassemblyPseudoPacketGet(int direction, Flow *f,
                                           TcpSession *ssn);
 ...
 */
static int FlowFinish(ThreadVars *tv, Flow *f, FlowWorkerThreadData *fw,
                      void *detect_thread) {
...
static void CheckWorkQueue(ThreadVars *tv, FlowWorkerThreadData *fw,
    ...
    while ((f = FlowQueuePrivateGetFromTop(fq)) != NULL) {
        ...
        FLOWLOCK_UNLOCK(f);
        if (fw->fls.spare_queue.len >=
            200) {  // TODO match to API? 200 = 2 * block size
            FlowSparePoolReturnFlow(f);
        } else {
            FlowQueuePrivatePrependFlow(&fw->fls.spare_queue, f);
        }
        // TODO 20200503 we can get here with use_cnt > 0. How does it work wrt
        // timeout? Should we not queue it? But what then?
    }
}
 ...
 */
static inline TmEcode FlowUpdate(ThreadVars *tv, FlowWorkerThreadData *fw,
                                 Packet *p) {
...
static TmEcode FlowWorkerThreadInit(ThreadVars *tv, const void *initdata,
                                    void **data) {
    FlowWorkerThreadData *fw = SCCalloc(1, sizeof(*fw));
    if (fw == NULL) return TM_ECODE_FAILED;

    SC_ATOMIC_INITPTR(fw->detect_thread);
    SC_ATOMIC_SET(fw->detect_thread, NULL);

    fw->local_bypass_pkts =
        StatsRegisterCounter("flow_bypassed.local_pkts", tv);
    fw->local_bypass_bytes =
        StatsRegisterCounter("flow_bypassed.local_bytes", tv);
    fw->both_bypass_pkts =
        StatsRegisterCounter("flow_bypassed.local_capture_pkts", tv);
    fw->both_bypass_bytes =
        StatsRegisterCounter("flow_bypassed.local_capture_bytes", tv);

    fw->cnt.flows_aside_needs_work =
        StatsRegisterCounter("flow.wrk.flows_evicted_needs_work", tv);
    fw->cnt.flows_aside_pkt_inject =
        StatsRegisterCounter("flow.wrk.flows_evicted_pkt_inject", tv);
    fw->cnt.flows_removed = StatsRegisterCounter("flow.wrk.flows_evicted", tv);
    fw->cnt.flows_injected =
        StatsRegisterCounter("flow.wrk.flows_injected", tv);
    ...
    if (DetectEngineEnabled()) {
        ...
        if (DetectEngineThreadCtxInit(tv, NULL, &detect_thread) !=
            TM_ECODE_OK) {
            FlowWorkerThreadDeinit(tv, fw);
            return TM_ECODE_FAILED;
        }
    ...
    if (OutputLoggerThreadInit(tv, initdata, &fw->output_thread) !=
        TM_ECODE_OK) {
        FlowWorkerThreadDeinit(tv, fw);
        return TM_ECODE_FAILED;
    }
    if (OutputFlowLogThreadInit(tv, NULL, &fw->output_thread_flow) !=
        TM_ECODE_OK) {
...
static inline void UpdateCounters(ThreadVars *tv, FlowWorkerThreadData *fw,
                                  const FlowTimeoutCounters *counters) {
...
static void FlowPruneFiles(Packet *p) {
    if (p->flow && p->flow->alstate) {
        Flow *f = p->flow;
        FileContainer *fc = AppLayerParserGetFiles(
            f, PKT_IS_TOSERVER(p) ? STREAM_TOSERVER : STREAM_TOCLIENT);
        if (fc != NULL) {
            FilePrune(fc);
        }
    }
...
static inline void FlowWorkerStreamTCPUpdate(ThreadVars *tv,
                                             FlowWorkerThreadData *fw,
...
static inline void FlowWorkerProcessLocalFlows(ThreadVars *tv,
    ...
    if (fw->fls.work_queue.len) {
        StatsAddUI64(tv, fw->cnt.flows_removed,
                     (uint64_t)fw->fls.work_queue.len);
...
static TmEcode FlowWorker(ThreadVars *tv, Packet *p, void *data) {
    ...
    SCLogDebug("packet %" PRIu64 " has flow? %s", p->pcap_cnt,
               p->flow ? "yes" : "no");
