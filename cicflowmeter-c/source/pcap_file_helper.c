...
static void PcapFileCallbackLoop(char *user, struct pcap_pkthdr *h,
                                 u_char *pkt);
...
void PcapFileCallbackLoop(char *user, struct pcap_pkthdr *h, u_char *pkt) {
    ...
    if (TmThreadsSlotProcessPkt(ptv->shared->tv, ptv->shared->slot, p) !=
        TM_ECODE_OK) {
...
const char *PcapFileGetFilename(void) { return pcap_filename; }
...
TmEcode PcapFileDispatch(PcapFileFileVars *ptv) {
    ...
    while (loop_result == TM_ECODE_OK) {
        ...
        int r =
            pcap_dispatch(ptv->pcap_handle, packet_q_len,
        ...
        if (unlikely(r == -1)) {
            ...
            SCLogError(SC_ERR_PCAP_DISPATCH,
                       "Pcap callback PcapFileCallbackLoop failed for %s",
                       ptv->filename);
...
static bool PeekFirstPacketTimestamp(PcapFileFileVars *pfv) {
    int r = pcap_next_ex(pfv->pcap_handle, &pfv->first_pkt_hdr,
                         &pfv->first_pkt_data);
    if (r <= 0 || pfv->first_pkt_hdr == NULL) {
        SCLogError(SC_ERR_PCAP_OPEN_OFFLINE,
                   "failed to get first packet timestamp. pcap_next_ex(): %d",
                   r);
        return false;
    }
...
TmEcode InitPcapFile(PcapFileFileVars *pfv) {
    ...
    if (pfv->shared != NULL && pfv->shared->bpf_string != NULL) {
        ...
        if (pcap_compile(pfv->pcap_handle, &pfv->filter,
                         pfv->shared->bpf_string, 1, 0) < 0) {
    ...
    if (!PeekFirstPacketTimestamp(pfv)) SCReturnInt(TM_ECODE_FAILED);
