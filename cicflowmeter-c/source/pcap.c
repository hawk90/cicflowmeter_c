 ...
 * which we do.
 */
typedef struct PCAP_STATS_64_ {
    uint64_t pcap_stats_recv;
    uint64_t pcap_stats_drop;
    uint64_t pcap_stats_if_drop;
} PCAP_STATS_64;

/**
 * \brief Structure to hold thread specific variables.
 */
typedef struct PCAP_THREAD_VARS_
{
    ...
    THREAD_VARS *thread_val;
    THREAD_MODULE_SLOT *slot;
...
static void pcap_callback_loop(char *user, struct pcap_pkthdr *h, u_char *pkt)
{
    Enter();

    struct timeval current_time;
    PCAP_THREAD_VARS *ptv = (PCAP_THREAD_VARS *)user;
    PACKET *p = get_packet_from_queue_or_alloc();
