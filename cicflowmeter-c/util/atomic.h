...
#define ATOMIC_INIT(name) \
    (name ## __atomic__) = 0
#define ATOMIC_INITPTR(name) \
    (name ## __atomic__) = NULL
...
#define ATOMIC_ADD(name, val) \
    atomic_fetch_add(&(name ## __atomic__), (val))
...
#define ATOMIC_SUB(name, val) \
    atomic_fetch_sub(&(name ## __atomic__), (val))
 ...
 */
#define ATOMIC_OR(name, val) \
    atomic_fetch_or(&(name ## __atomic__), (val))
...
#define ATOMIC_AND(name, val) \
    atomic_fetch_and(&(name ## __atomic__), (val))
