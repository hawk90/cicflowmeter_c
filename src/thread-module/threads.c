...
TmEcode TmThreadsSlotVarRun(ThreadVars *tv, Packet *p, TmSlot *slot) {
    for (TmSlot *s = slot; s != NULL; s = s->slot_next) {
        ...
        while (tv->decode_pq.top != NULL) {
            ...
            if (unlikely(extra_p == NULL)) continue;
...
static int TmThreadTimeoutLoop(ThreadVars *tv, TmSlot *s) {
    ...
    if (tv->stream_pq == NULL || fw_slot == NULL) {
        SCLogDebug("not running TmThreadTimeoutLoop %p/%p", tv->stream_pq,
                   fw_slot);
        return r;
    }
    ...
    while (1) {
        ...
        if (len > 0) {
            while (len--) {
                ...
                SCMutexUnlock(&tv->stream_pq->mutex_q);
                if (likely(p)) {
                    if ((r = TmThreadsSlotProcessPkt(tv, fw_slot, p) !=
                             TM_ECODE_OK)) {
...
static void *TmThreadsSlotPktAcqLoop(void *td) {
    ...
    if (tv->thread_setup_flags != 0) TmThreadSetupOptions(tv);
    ...
    if (s == NULL || s->PktAcqLoop == NULL || tv->tmqh_in == NULL ||
        tv->tmqh_out == NULL) {
        ...
        if (slot->SlotThreadInit != NULL) {
            ...
            if (r != TM_ECODE_OK) {
                if (r == TM_ECODE_DONE) {
                    EngineDone();
                    TmThreadsSetFlag(
                        tv, THV_CLOSED | THV_INIT_DONE | THV_RUNNING_DONE);
                    goto error;
                } else {
                    TmThreadsSetFlag(tv, THV_CLOSED | THV_RUNNING_DONE);
                    goto error;
 ...
 */
static TmEcode TmThreadSetSlots(ThreadVars *tv, const char *name,
                                void *(*fn_p)(void *)) {
    if (name == NULL) {
        if (fn_p == NULL) {
            printf(
...
void TmThreadSetPrio(ThreadVars *tv) {
    ...
    if (0 == SetThreadPriority(GetCurrentThread(), tv->thread_priority)) {
        SCLogError(SC_ERR_THREAD_NICE_PRIO,
                   "Error setting priority for "
                   "thread %s: %s",
...
TmEcode TmThreadSetCPU(ThreadVars *tv, uint8_t type) {
    if (!threading_set_cpu_affinity) return TM_ECODE_OK;
...
ThreadVars *TmThreadCreate(const char *name, const char *inq_name,
    ...
    if (unlikely(tv == NULL)) goto error;
...
ThreadVars *TmThreadCreatePacketHandler(const char *name, const char *inq_name,
    ...
    tv = TmThreadCreate(name, inq_name, inqh_name, outq_name, outqh_name, slots,
                        NULL, 0);
...
ThreadVars *TmThreadCreateMgmtThreadByName(const char *name, const char *module,
    ...
    tv = TmThreadCreate(name, NULL, NULL, NULL, NULL, "management", NULL,
                        mucond);
...
static int TmThreadKillThread(ThreadVars *tv) {
    ...
    if (!(TmThreadsCheckFlag(tv, THV_CLOSED))) {
        ...
        if (tv->inq != NULL) {
            for (int i = 0; i < (tv->inq->reader_cnt + tv->inq->writer_cnt);
                 i++) {
...
again:
    ...
    if ((cur_ts.tv_sec - start_ts.tv_sec) > 60) {
        SCLogWarning(SC_ERR_SHUTDOWN,
                     "unable to get all packet threads "
...
void TmThreadClearThreadsFamily(int family) {
    ...
    if ((family < 0) || (family >= TVT_MAX)) return;
...
void TmThreadTestThreadUnPaused(ThreadVars *tv) {
    while (TmThreadsCheckFlag(tv, THV_PAUSE)) {
        ...
        if (TmThreadsCheckFlag(tv, THV_KILL)) break;
    }
...
again:
    ...
    SCLogNotice("all %" PRIu16 " packet processing threads, %" PRIu16
                " management "
...
uint32_t TmThreadCountThreadsByTmmFlags(uint8_t flags) {
    ...
    for (int i = 0; i < TVT_MAX; i++) {
        ThreadVars *tv = tv_root[i];
        while (tv != NULL) {
            if ((tv->tmm_flags & flags) == flags) cnt++;
...
static void TmThreadDoDumpSlots(const ThreadVars *tv) {
    for (TmSlot *s = tv->tm_slots; s != NULL; s = s->slot_next) {
        ...
        SCLogNotice("tv %p: -> slot %p tm_id %d name %s", tv, s, s->tm_id,
                    m->name);
    }
}
    ...
    for (int i = 0; i < TVT_MAX; i++) {
        ...
        while (tv != NULL) {
            ...
            if (tv->inq && tv->stream_pq == tv->inq->pq) {
                ...
                for (Packet *xp = tv->stream_pq_local->top; xp != NULL;
                     xp = xp->next) {
                    SCLogNotice(
                        "tv %p: ==> stream_pq_local: pq.len %u packet src %s",
                        tv, tv->stream_pq_local->len,
                        PktSrcToString(xp->pkt_src));
                }
            }
            for (Packet *xp = tv->decode_pq.top; xp != NULL; xp = xp->next) {
                SCLogNotice(
                    "tv %p: ==> decode_pq: decode_pq.len %u packet src %s", tv,
...
void TmThreadsListThreads(void) {
    ...
    for (size_t s = 0; s < thread_store.threads_size; s++) {
        ...
        if (t->tv) {
            ThreadVars *tv = t->tv;
            const uint32_t flags = SC_ATOMIC_GET(tv->flags);
            SCLogNotice("tv %p type %u name %s tmm_flags %02X flags %X", tv,
                        tv->type, tv->name, tv->tmm_flags, flags);
        }
    }
...
int TmThreadsRegisterThread(ThreadVars *tv, const int type) {
    ...
    void *newmem =
        SCRealloc(thread_store.threads,
                  ((thread_store.threads_size + STEP) * sizeof(Thread)));
    BUG_ON(newmem == NULL);
    thread_store.threads = newmem;
    memset((uint8_t *)thread_store.threads +
               (thread_store.threads_size * sizeof(Thread)),
           0x00, STEP * sizeof(Thread));
...
void TmThreadsGetMinimalTimestamp(struct timeval *ts) {
    ...
    for (s = 0; s < thread_store.threads_size; s++) {
        ...
        if (!(timercmp(&t->pktts, &nullts, ==))) {
            ...
            if (t->sys_sec_stamp + 1 < (uint32_t)systs.tv_sec) continue;
...
uint16_t TmThreadsGetWorkerThreadMax() {
    ...
    if (thread_max == 0) thread_max = ncpus * threading_detect_ratio;
    if (thread_max < 1) thread_max = 1;
    if (thread_max > 1024) {
        SCLogWarning(SC_ERR_RUNMODE,
                     "limited number of 'worker' threads to 1024. Wanted %d",
                     thread_max);
        thread_max = 1024;
    }
    return thread_max;
...
int TmThreadsInjectPacketsById(Packet **packets, const int id) {
    if (id <= 0 || id > (int)thread_store.threads_size) return 0;
    ...
    if (tv == NULL || tv->stream_pq == NULL) return 0;
