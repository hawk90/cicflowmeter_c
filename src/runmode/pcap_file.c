...
const char *get_default_pcap_file_runmode(void) { return "autofp" }

void runmode_pcap_file_fegister(void) {
    RunModeRegisterNewRunMode(RUNMODE_PCAP_FILE, "single",
                              "Single threaded pcap file mode",
                              RunModeFilePcapSingle);
    RunModeRegisterNewRunMode(
        RUNMODE_PCAP_FILE, "autofp",
 ...
 */
int runmode_file_pcap_single(void) {
    ...
    /* create the threads */
    ThreadVars *tv =
        TmThreadCreatePacketHandler(tname, "packetpool", "packetpool",
 ...
 *           exit()).
 */
int runmode_file_pcap_auto_fp(void) {
    SCEnter();
    char tname[TM_THREAD_NAME_MAX];
    ...
    if (thread_max > 1024) thread_max = 1024;
    ...
    for (thread = 0; thread < (uint16_t)thread_max; thread++) {
        snprintf(tname, sizeof(tname), "%s#%02u", thread_name_workers,
                 thread + 1);
