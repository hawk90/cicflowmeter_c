...
const char *RunModeIdsGetDefaultMode(void) { return "autofp"; }

int runmode_ids_pcap_workers(void);
...
static void pcap_deref_config(void *conf) {
...
static void *parse_pcap_config(const char *iface) {
    ...
    /* If there is no setting for current interface use default one as main
     * iface */
    if (if_root == NULL) {
        if_root = if_default;
        if_default = NULL;
    ...
    if (ConfGetChildValueWithDefault(if_root, if_default, "threads",
                                     &threadsstr) != 1) {
        aconf->threads = 1;
    } else {
        if (threadsstr != NULL) {
            if (StringParseInt32(&aconf->threads, 10, 0,
    ...
    if (aconf->buffer_size == 0) {
        ...
        int ret;
        ret = ConfGetChildValueWithDefault(if_root, if_default, "buffer-size",
                                           &s_limit);
        if (ret == 1 && s_limit) {
            uint64_t bsize = 0;
    ...
    if (aconf->bpf_filter == NULL) {
        ...
        if (ConfGetChildValueWithDefault(if_root, if_default, "bpf-filter",
                                         &tmpbpf) != 1) {
    ...
    if (ConfGetChildValueWithDefault(if_root, if_default, "checksum-checks",
                                     &tmpctype) == 1) {
        if (strcmp(tmpctype, "auto") == 0) {
            ...
            SCLogError(SC_ERR_INVALID_ARGUMENT,
                       "Invalid value for checksum-checks for %s",
                       aconf->iface);
        }
    }

    aconf->promisc = LIBPCAP_PROMISC;
    if (ConfGetChildValueBoolWithDefault(if_root, if_default, "promisc",
                                         &promisc) != 1) {
    ...
    aconf->snaplen = 0;
    if (ConfGetChildValueIntWithDefault(if_root, if_default, "snaplen",
                                        &snaplen) != 1) {
...
static int get_threads_count_pcap_config(void *conf) {
    PcapIfaceConfig *pfp = (PcapIfaceConfig *)conf;
    return pfp->threads;
}
...
int runmode_ids_pcap_single(void) {
    ...
    ret = RunModeSetLiveCaptureSingle(ParsePcapConfig, PcapConfigGeThreadsCount,
                                      "ReceivePcap", "DecodePcap",
...
int runmode_ids_pcap_auto_fp(void) {
    ...
    ret = RunModeSetLiveCaptureAutoFp(ParsePcapConfig, PcapConfigGeThreadsCount,
                                      "ReceivePcap", "DecodePcap",
...
int runmode_ids_pcap_workers(void) {
    ...
    ret = RunModeSetLiveCaptureWorkers(
        ParsePcapConfig, PcapConfigGeThreadsCount, "ReceivePcap", "DecodePcap",
