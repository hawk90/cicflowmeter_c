...
static inline PktPool *GetThreadPacketPool(void) { return &thread_pkt_pool; }
...
static int PacketPoolIsEmpty(PktPool *pool) {
    ...
    if (pool->head || pool->return_stack.head) return 0;
...
void PacketPoolWait(void) {
    ...
    while (PacketPoolIsEmpty(my_pool)) cc_barrier();
}
...
void PacketPoolWaitForN(int n) {
    ...
    while (1) {
        ...
        if (my_pool->return_stack.head != NULL) {
            SCMutexLock(&my_pool->return_stack.mutex);
            /* Move all the packets from the locked return stack to the local
             * stack. */
            if (pp) {
            ...
            SCMutexLock(&my_pool->return_stack.mutex);
            SC_ATOMIC_ADD(my_pool->return_stack.sync_now, 1);
            SCCondWait(&my_pool->return_stack.cond,
                       &my_pool->return_stack.mutex);
            SCMutexUnlock(&my_pool->return_stack.mutex);
        }
    }
}
...
void PacketPoolReturnPacket(Packet *p) {
    ...
    if (pool == my_pool) {
        ...
        if (pending_pool == NULL) {
            ...
            if (SC_ATOMIC_GET(pool->return_stack.sync_now) ||
                my_pool->pending_count > max_pending_return_packets) {
...
void PacketPoolInit(void) {
    ...
    for (i = 0; i < max_pending_packets; i++) {
        ...
        if (unlikely(p == NULL)) {
            FatalError(SC_ERR_FATAL,
                       "Fatal error encountered while allocating a packet. "
                       "Exiting...");
        }
    ...
    //        max_pending_packets,
    //        (uintmax_t)(max_pending_packets*SIZE_OF_PACKET));
}
...
void TmqhOutputPacketpool(ThreadVars *t, Packet *p) {
    ...
    SCEnter();
    SCLogDebug("Packet %p, p->root %p, alloced %s", p, p->root,
               p->flags & PKT_ALLOC ? "true" : "false");

    if (IS_TUNNEL_PKT(p)) {
        SCLogDebug("Packet %p is a tunnel packet: %s", p,
                   p->root ? "upper layer" : "tunnel root");
        ...
        if (IS_TUNNEL_ROOT_PKT(p)) {
            ...
            if (outstanding == 0 && p->root &&
                ...
                SCLogDebug(
                    ...
                    "outstanding > 0 (%u)",
                    (p->root && IS_TUNNEL_PKT_VERDICTED(p->root)) ? "true"
                                                                  : "false",
                    outstanding);
    ...
    if (proot == true) {
        SCLogDebug("getting rid of root pkt... alloc'd %s",
                   p->root->flags & PKT_ALLOC ? "true" : "false");
 ...
 *         used by threads that have failed, and wants to return the packets
 * back to the packetpool.
 *
...
void TmqhReleasePacketsToPacketPool(PacketQueue *pq) {
    ...
    while ((p = PacketDequeue(pq)) != NULL) TmqhOutputPacketpool(NULL, p);
