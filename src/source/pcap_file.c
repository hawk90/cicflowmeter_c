...
static void CleanupPcapFileFromThreadVars(PcapFileThreadVars *tv,
...
void CleanupPcapFileFromThreadVars(PcapFileThreadVars *tv,
                                   PcapFileFileVars *pfv) {
...
void CleanupPcapDirectoryFromThreadVars(PcapFileThreadVars *tv,
                                        PcapFileDirectoryVars *ptv) {
...
void TmModuleReceivePcapFileRegister(void) {
    ...
    tmm_modules[TMM_RECEIVEPCAPFILE].ThreadExitPrintStats =
        ReceivePcapFileThreadExitStats;
...
TmEcode ReceivePcapFileLoop(ThreadVars *tv, void *data, void *slot) {
    ...
    if (ptv->is_directory == 0) {
        ...
        SCLogInfo("Starting directory run for %s",
                  ptv->behavior.directory->filename);
...
TmEcode ReceivePcapFileThreadInit(ThreadVars *tv, const void *initdata,
                                  void **data) {
    SCEnter();
    ...
    if (PcapDetermineDirectoryOrFile((char *)initdata, &directory) ==
        TM_ECODE_FAILED) {
        CleanupPcapFileThreadVars(ptv);
        SCReturnInt(TM_ECODE_OK);
    }
...
void ReceivePcapFileThreadExitStats(ThreadVars *tv, void *data) {
    ...
    if (data != NULL) {
        ...
        if (pcap_g.conf_checksum_mode == CHECKSUM_VALIDATION_AUTO &&
            ...
            uint64_t chrate =
                pcap_g.cnt / SC_ATOMIC_GET(pcap_g.invalid_checksums);
        ...
        SCLogNotice("Pcap-file module read %" PRIu64 " files, %" PRIu64
...
TmEcode DecodePcapFileThreadInit(ThreadVars *tv, const void *initdata,
                                 void **data) {
    SCEnter();
    DecodeThreadVars *dtv = NULL;
    dtv = DecodeThreadVarsAlloc(tv);

    if (dtv == NULL) SCReturnInt(TM_ECODE_FAILED);
...
TmEcode DecodePcapFileThreadDeinit(ThreadVars *tv, void *data) {
    if (data != NULL) DecodeThreadVarsFree(tv, data);
    SCReturnInt(TM_ECODE_OK);
}
