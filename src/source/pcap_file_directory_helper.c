...
TmEcode PcapRunStatus(PcapFileDirectoryVars *ptv) {
    if (RunModeUnixSocketIsActive()) {
        TmEcode done =
            UnixSocketPcapFile(TM_ECODE_OK, &ptv->shared->last_processed);
...
TmEcode PcapDetermineDirectoryOrFile(char *filename, DIR **directory) {
    ...
    if (temp_dir == NULL) {  // if null, our filename may just be a normal file
        switch (errno) {
            ...
            case ENOTDIR:  // no error checking the directory, just is a plain
                           // file
...
#ifdef OS_WIN32
    if ((ret = _stat(file, &buf)) != 0) return ret;
#else
    if ((ret = stat(file, &buf)) != 0) return ret;
#endif
...
TmEcode PcapDirectoryInsertFile(PcapFileDirectoryVars *pv,
    ...
    if (TAILQ_EMPTY(&pv->directory_content)) {
        ...
        while (file_to_compare != NULL) {
            if (CompareTimes(&file_to_add->modified_time,
                             &file_to_compare->modified_time) < 0) {
...
TmEcode PcapDirectoryPopulateBuffer(PcapFileDirectoryVars *pv,
    ...
    while ((dir = readdir(pv->directory)) != NULL) {
        ...
        written =
            snprintf(pathbuff, PATH_MAX, "%s/%s", pv->filename, dir->d_name);
        ...
        if (written <= 0 || written >= PATH_MAX) {
            ...
            if (PcapDirectoryGetModifiedTime(pathbuff, &temp_time) == 0) {
                SCLogDebug("%" PRIuMAX " < %" PRIuMAX "(%s) < %" PRIuMAX ")",
                           (uintmax_t)SCTimespecAsEpochMillis(
                               &pv->shared->last_processed),
                ...
                if (CompareTimes(&temp_time, &pv->shared->last_processed) <=
                    0) {
                ...
                SCLogDebug("Unable to get modified time on %s, skipping",
                           pathbuff);
                continue;
            }
        ...
        while (status == TM_ECODE_OK && !TAILQ_EMPTY(&pv->directory_content)) {
            ...
            if (unlikely(current_file == NULL)) {
                ...
                SCLogWarning(SC_ERR_PCAP_DISPATCH,
                             "Current file filename was null");
            } else {
                ...
                if (InitPcapFile(pftv) == TM_ECODE_FAILED) {
                    ...
                    SCLogInfo("Processed file %s, processed up to %" PRIuMAX,
                              current_file->filename,
                              (uintmax_t)SCTimespecAsEpochMillis(
                                  &current_file->modified_time));

                    if (CompareTimes(&current_file->modified_time,
                                     &last_time_seen) > 0) {
...
TmEcode PcapDirectoryDispatch(PcapFileDirectoryVars *ptv) {
    ...
    while (status == TM_ECODE_OK) {
        ...
        SCLogInfo(
            ...
            ptv->filename,
            (uintmax_t)SCTimespecAsEpochMillis(&ptv->shared->last_processed),
            (uintmax_t)SCTimespecAsEpochMillis(&older_than));
        status = PcapDirectoryDispatchForTimeRange(ptv, &older_than);
        if (ptv->should_loop && status == TM_ECODE_OK) {
            sleep(poll_seconds);
            // update our status based on suricata control flags or unix command
            // socket
            status = PcapRunStatus(ptv);
            if (status == TM_ECODE_OK) {
                SCLogDebug("Checking if directory %s still exists",
                           ptv->filename);
                // check directory
                if (PcapDetermineDirectoryOrFile(
                        ptv->filename, &directory_check) == TM_ECODE_FAILED) {
    ...
    if (status == TM_ECODE_FAILED) {
        SCLogError(SC_ERR_PCAP_DISPATCH, "Directory %s run mode failed",
                   ptv->filename);
