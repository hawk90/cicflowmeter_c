static int decode_icmpv4_test01(void) {
    uint8_t raw[] = {0x08, 0x00, 0x78, 0x47, 0xfc, 0x55, 0x00, 0x04, 0x52, 0xab,
                     0x86, 0x4a, 0x84, 0x50, 0x0e, 0x00, 0xab, 0xab, 0xab, 0xab,
    ...
    memset(&ip4_hdr, 0, sizeof(IPV4_HDR_T));
    memset(&thread, 0, sizeof(THREAD_T));
    memset(pkt, 0, SIZE_OF_PACKET);
    memset(&thread_vars, 0, sizeof(THREAD_VARS_T));

    init_flow_config(FLOW_QUIET);

    pkt->src.family = AF_INET;
    pkt->dst.family = AF_INET;
    pkt->src.addr_data32[0] = UTHSetIPv4Address("4.3.2.1");
    pkt->dst.addr_data32[0] = UTHSetIPv4Address("1.2.3.4");

    ip4_hdr.s_ip_src.s_addr = p->src.addr_data32[0];
    ip4_hdr.s_ip_dst.s_addr = p->dst.addr_data32[0];
    pkt->ip4_hdr = &ip4_hdr;

    DecodeICMPV4(&thread, &thread_vars, pkt, raw, sizeof(raw));

    if (NULL != pkt->icmpv4_h) {
        if (pkt->icmpv4_hdr->type == 8 && pkt->icmpv4_hdr->code == 0) {
            rc = 1;
        }
    }
...
static int DecodeICMPV4test02(void) {
    uint8_t raw_icmpv4[] = {
        0x00, 0x00, 0x57, 0x64, 0xfb, 0x55, 0x00, 0x03, 0x43, 0xab, 0x86, 0x4a,
        0xf6, 0x49, 0x02, 0x00, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
...
static int DecodeICMPV4test03(void) {
    uint8_t raw_icmpv4[] = {
        0x0b, 0x00, 0x6a, 0x3d, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x3c,
        0x64, 0x15, 0x00, 0x00, 0x01, 0x11, 0xde, 0xfd, 0xc0, 0xa8, 0x01, 0x0d,
        0xd1, 0x55, 0xe3, 0x93, 0x8b, 0x12, 0x82, 0xaa, 0x00, 0x28, 0x7c, 0xdd};
    Packet *p = SCMalloc(SIZE_OF_PACKET);
    if (unlikely(p == NULL)) return 0;
    ThreadVars tv;
    DecodeThreadVars dtv;
    ...
    if (p->icmpv4h->type != 11 || p->icmpv4h->code != 0) {
        printf("p->icmpv4h->type %u, p->icmpv4h->code %u: ", p->icmpv4h->type,
               p->icmpv4h->code);
        goto end;
    }
...
static int DecodeICMPV4test04(void) {
    uint8_t raw_icmpv4[] = {
        0x03, 0x0a, 0x36, 0xc3, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x3c,
        0x62, 0xee, 0x40, 0x00, 0x33, 0x06, 0xb4, 0x8f, 0xc0, 0xa8, 0x01, 0x0d,
        0x58, 0x60, 0x16, 0x29, 0xb1, 0x0a, 0x00, 0x32, 0x3e, 0x36, 0x38, 0x7c,
        0x00, 0x00, 0x00, 0x00, 0xa0, 0x02, 0x16, 0xd0, 0x72, 0x04, 0x00, 0x00,
...
static int DecodeICMPV4test05(void) {
    uint8_t raw_icmpv4[] = {
        0x0b, 0x00, 0x5c, 0x46, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x30,
        0x02, 0x17, 0x40, 0x00, 0x01, 0x06, 0xd6, 0xbd, 0xc0, 0xa8, 0x02, 0x05,
        0x3d, 0x23, 0xa1, 0x23, 0x04, 0x18, 0x00, 0x50, 0xd2, 0x08, 0xc2, 0x48,
    };
    Packet *p = SCMalloc(SIZE_OF_PACKET);
    if (unlikely(p == NULL)) return 0;
    ThreadVars tv;
...
static int ICMPV4CalculateValidChecksumtest05(void) {
    ...
    uint8_t raw_icmpv4[] = {
        0x08, 0x00, 0xab, 0x9b, 0x7f, 0x2b, 0x05, 0x2c, 0x3f, 0x72, 0x93,
        0x4a, 0x00, 0x4d, 0x0a, 0x00, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
    ...
    return (csum == ICMPV4CalculateChecksum((uint16_t *)raw_icmpv4,
                                            sizeof(raw_icmpv4)));
}
...
static int ICMPV4CalculateInvalidChecksumtest06(void) {
    ...
    uint8_t raw_icmpv4[] = {
        ...
        0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23,
        0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e,
        0x2f, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x38};
...
static int ICMPV4InvalidType07(void) {
    uint8_t raw_icmpv4[] = {
        0xff, 0x00, 0xab, 0x9b, 0x7f, 0x2b, 0x05, 0x2c, 0x3f, 0x72, 0x93,
        0x4a, 0x00, 0x4d, 0x0a, 0x00, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
        0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
        0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23,
        0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e,
